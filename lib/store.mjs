...
CREATE TABLE IF NOT EXISTS conversations (
  ...
  file_name       TEXT,   -- the json/ filename last indexed for this conversation
  file_mtime      REAL,   -- mtimeMs of that file when last indexed
...
export function openStore(dataDir) {
  ...
  // Add columns that may be missing in DBs created before they were introduced
  for (const col of [
    'ALTER TABLE conversations ADD COLUMN file_name TEXT',
    'ALTER TABLE conversations ADD COLUMN file_mtime REAL',
  ]) {
...
/**
 * Scan the json/ directory and index conversations into the store.
 * Uses file mtime caching — skips files whose name + mtime haven't changed
 * since the last index run. Only reads and parses new or modified files.
 *
 * Returns { added, updated, skipped, total } counts.
 */
export function indexFromDisk(db, dataDir) {
  ...
  // Build cache: filename → { id, mtime } from the last index run.
  // Only entries where file_name is set and on_disk = 1 qualify.
  const cacheRows = db.prepare(
    'SELECT id, file_name, file_mtime FROM conversations WHERE file_name IS NOT NULL AND on_disk = 1'
  ).all();
  const cache = new Map();
  for (const row of cacheRows) {
    cache.set(row.file_name, { id: row.id, mtime: row.file_mtime });
  }

  // Separate files into "must read" and "skip"
  const toRead = [];
  let skipped = 0;

  for (const file of files) {
    const fstat = statSync(join(jsonDir, file));
    const mtime = fstat.mtimeMs;
    const cached = cache.get(file);

    if (cached && mtime <= cached.mtime + 1) {
      // Same filename, mtime hasn't advanced → file unchanged, skip read
      skipped++;
    } else {
      toRead.push({ file, mtime });
    }
  }

  console.log(`  Scanning ${files.length} JSON files (${toRead.length} to index, ${skipped} unchanged)...`);

  const insert = db.prepare(
    `INSERT INTO conversations (id, title, create_time, update_time, gizmo_id, _raw, on_disk, file_name, file_mtime)
     VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
     ON CONFLICT(id) DO UPDATE SET
       title=excluded.title, create_time=excluded.create_time,
       ...
       file_name=excluded.file_name, file_mtime=excluded.file_mtime`
  );
  ...
  if (toRead.length > 0) {
    db.exec('BEGIN');
    for (const { file, mtime } of toRead) {
      try {
        ...
        insert.run(id, conv.title || 'Untitled', conv.create_time || null, updateTime,
          conv.gizmo_id || null, raw, file, mtime);
  ...
  // Mark all scanned files as on_disk; clear the flag for any that disappeared
  const onDiskIds = new Set();
  for (const { id } of cache.values()) onDiskIds.add(id);
  // Add IDs from freshly-read files too
  for (const { file } of toRead) {
    const cached = cache.get(file);
    if (cached) onDiskIds.add(cached.id);
  }

  // Mark all conversations with on_disk=1 that are in the cache or newly indexed.
  // Mark on_disk=0 for conversations whose file_name isn't in the current file list.
  const currentFiles = new Set(files);
  db.exec('BEGIN');
  const markOnDisk = db.prepare('UPDATE conversations SET on_disk = 1 WHERE file_name = ?');
  const markOffDisk = db.prepare('UPDATE conversations SET on_disk = 0 WHERE file_name = ? AND file_name IS NOT NULL');
  const knownFileNames = db.prepare('SELECT DISTINCT file_name FROM conversations WHERE file_name IS NOT NULL').all();

  for (const row of knownFileNames) {
    if (currentFiles.has(row.file_name)) {
      markOnDisk.run(row.file_name);
    } else {
      markOffDisk.run(row.file_name);
    }
  }
  db.exec('COMMIT');
